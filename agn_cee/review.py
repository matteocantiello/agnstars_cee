...
# The gas-hardening "ladder" now lives in bbh.py (single source of truth). Re-export
# the names this module and the notebooks reference so existing call sites keep working.
from .bbh import (HARDENING_MODELS, kkss08_I2_phi, companion_wake_fraction,
                  cd_windtunnel, f_shared_bondi, gas_loss_rate_model,
                  dadt_model, merger_time_model)
...
def fig_characteristic_strain(model, m1=10 * cst.MSUN, m2=10 * cst.MSUN, distance=100 * cst.MPC):
    ...
    # explicit order so column 2 reads: fiducial, model range, vacuum (legend fills column-major)
    handles, labels = ax.get_legend_handles_labels()
    order = ["LISA", "DECIGO", "LVK", "BBH in immortal (fiducial)",
             "gas-hardening model range", "same BBH, vacuum"]
