...
def fig_characteristic_strain(model, m1=10 * cst.MSUN, m2=10 * cst.MSUN, distance=100 * cst.MPC):
    ...
    ax.set_xlim(1e-4, 1e4); ax.set_ylim(1e-26, 5*1e-18)
    ax.set_xlabel("frequency [Hz]"); ax.set_ylabel(r"characteristic strain $h_c$")
    leg = ax.legend(fontsize=8, loc="upper right", ncol=2, frameon=True, framealpha=0.85)
    leg.get_frame().set_edgecolor("none")
    ax.text(0.75, 0.05, f"10+10 M$_\\odot$ at {distance/cst.MPC:.0f} Mpc",
...
# --------------------------------------------------------------------------- #
def fig_spiral_trajectories(model, m1=10 * cst.MSUN, m2=10 * cst.MSUN):
    """Schematic spiral-in trajectories with the region where the analytic
    approximations break shaded pink.

    (a) Case A: a single CO spirals from the stellar surface (R_shock) to the core;
    the pink disk marks r < r(M_star=m_BH), where the CO locally dominates the
    enclosed mass and the test-particle treatment fails -- a small central core.
    (b) Case B: the internal BBH orbit hardens to merger deep in the core; here
    R_BHL,i/r_i > 1 throughout, so the *entire* inspiral lies in the breakdown
    (3D-simulation) regime. The winding is schematic (the true inspirals involve
    hundreds-to-thousands of orbits).
    """
    Rsun = cst.RSUN
    PINK, DPINK = "#ff5c8a", "#b3004d"

    def logspiral(r_out, r_in, nturn, phase=0.0, n=2400):
        th = np.linspace(0.0, 2.0 * np.pi * nturn, n)
        rr = r_out * (r_in / r_out) ** (th / th[-1])
        return rr * np.cos(th + phase), rr * np.sin(th + phase)

    fig, (axA, axB) = plt.subplots(1, 2, figsize=(DCOL, 3.8))

    # ---- Case A: single CO, surface -> M_star(<r)=m_BH core ----
    sol = inspiral.spiral_in_time(model, m1, model.r_shock)
    Rstar, rbreak = model.r_shock / Rsun, sol["r_final"] / Rsun
    xA, yA = logspiral(Rstar, rbreak, 6.0)
    axA.add_patch(plt.Circle((0, 0), Rstar, fc="0.93", ec="0.55", lw=1.3, zorder=0))
    axA.add_patch(plt.Circle((0, 0), rbreak, fc=PINK, ec="none", alpha=0.75, zorder=1))
    axA.plot(xA, yA, color=C[6], lw=0.9, zorder=2)
    axA.plot([xA[0]], [yA[0]], "o", color=C[6], ms=6, zorder=3)
    axA.annotate("capture", (xA[0], yA[0]), textcoords="offset points",
                 xytext=(3, 5), fontsize=7.5, color=C[6])
    axA.plot(0, 0, "+", color="k", ms=8, mew=1.4, zorder=4)
    L = Rstar * 1.13
    axA.set_xlim(-L, L); axA.set_ylim(-L, L); axA.set_aspect("equal")
    axA.set_title(r"(a) single compact object", fontsize=9.5)
    axA.annotate(r"$M_\star(<r)\lesssim m_{\rm BH}$", xy=(0, rbreak),
                 xytext=(0.30 * L, 0.58 * L), color=DPINK, fontsize=7.5, ha="center",
                 arrowprops=dict(arrowstyle="->", color=DPINK, lw=0.9))
    axA.text(-0.97 * L, 0.86 * L, "AGN star", color="0.45", fontsize=8)
    axA.set_xlabel(r"$x$ [R$_\odot$]"); axA.set_ylabel(r"$y$ [R$_\odot$]")

    # ---- Case B: internal BBH orbit. Solid = inspiral within the breakdown
    #      region (pink, R_BHL/r>1); dotted = the prior, wider trajectory by which
    #      the BHs reached the core, continuing out into the stellar interior (gray).
    M = m1 + m2
    a0, aend = cst.RSUN, bbh.a_isco(m1, m2)
    s_dot = (a0 * max(m1, m2) / M) / Rsun           # current half-separation (= pink boundary)
    s_in = (aend * max(m1, m2) / M) / Rsun          # merger
    s_out = s_dot * 2.8                             # arrived from further out (beyond the frame)
    nturn_in = 8.0
    ntot = nturn_in * (1.0 + np.log10(s_out / s_dot) / np.log10(s_dot / s_in))
    th = np.linspace(0.0, 2.0 * np.pi * ntot, 6000)
    rr = s_out * (s_in / s_out) ** (th / th[-1])     # one spiral: s_out -> s_in
    i_dot = int(np.argmin(np.abs(rr - s_dot)))       # split: dotted (outer) | solid (inner)
    r_pink = s_dot                                   # breakdown boundary (R_BHL/r=1 at a0)
    Lb = r_pink * 1.45                               # margin so the gray + dotted arrival show
    axB.set_facecolor("0.93")                        # whole panel is inside the star
    axB.add_patch(plt.Circle((0, 0), r_pink, fc=PINK, ec="none", alpha=0.5, zorder=1))
    for ph, col, lab in [(-th[i_dot], C[6], r"$m_1$"), (-th[i_dot] + np.pi, C[4], r"$m_2$")]:
        x, y = rr * np.cos(th + ph), rr * np.sin(th + ph)
        axB.plot(x[:i_dot + 1], y[:i_dot + 1], color=col, lw=0.9, zorder=2, label=lab)  # arrival (solid)
        axB.plot(x[i_dot:], y[i_dot:], color=col, lw=0.7, ls=":", zorder=3)             # inspiral (dotted)
        axB.plot([x[i_dot]], [y[i_dot]], "o", color=col, ms=6, zorder=4)              # current pos.
    axB.plot(0, 0, "+", color="k", ms=8, mew=1.4, zorder=4)
    axB.annotate("merger", (0, 0), textcoords="offset points", xytext=(4, 5), fontsize=7.5)
    axB.set_xlim(-Lb, Lb); axB.set_ylim(-Lb, Lb); axB.set_aspect("equal")
    axB.set_title(r"(b) internal BBH orbit", fontsize=9.5)
    axB.text(0, Lb * 0.93, r"(deep in stellar core, $r\ll R_\star$)", color="0.45",
             ha="center", va="top", fontsize=7.0, zorder=5,
             bbox=dict(boxstyle="round,pad=0.15", fc="0.93", ec="none"))
    axB.text(0, -Lb * 0.93, r"$R_{\rm BHL}/r\gtrsim1$: needs 3D simulation",
             color=DPINK, ha="center", va="bottom", fontsize=7.2, zorder=5,
             bbox=dict(boxstyle="round,pad=0.15", fc="0.93", ec="none"))
    axB.set_xlabel(r"$x$ [R$_\odot$]")
    leg = axB.legend(fontsize=8, loc="upper right", framealpha=0.9)
